        ...
        <View style={styles.sheet}>
          ...
          {done ? (
            <View style={styles.doneBox}>
              <Ionicons name="checkmark-circle" size={48} color={Afylo.green} />
            ...
            <>
              ...
              {REPORT_REASONS.map((r, i) => (
                <Pressable key={r} onPress={() => setDone(true)} style={[styles.row, i > 0 && styles.rowBorder]}>
                  <Text style={[styles.rowText, i === 0 && { color: Afylo.live, fontFamily: Font.semibold }]}>{r}</Text>
...
const styles = StyleSheet.create({
  ...
  sheet: { backgroundColor: Afylo.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 16, paddingBottom: 30 },
  grip: { width: 40, height: 4, borderRadius: 2, backgroundColor: Afylo.border, alignSelf: 'center', marginBottom: 14 },
  title: { ...Type.subtitle, color: Afylo.text },
  sub: { ...Type.small, color: Afylo.textDim, marginTop: 2, marginBottom: 8 },
  row: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 14 },
  rowBorder: { borderTopWidth: 1, borderTopColor: Afylo.border },
  rowText: { ...Type.body, color: Afylo.text },
  doneBox: { alignItems: 'center', paddingVertical: 16 },
  doneTitle: { ...Type.title, fontSize: 19, color: Afylo.text, marginTop: 12 },
