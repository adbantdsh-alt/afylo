...
import { listAccounts, removeAccount, setAddingAccount, switchAccount, upsertTokens, type SavedAccount } from '@/lib/accounts';
...
export function AccountSwitcher({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  // Ajouter/créer un compte : on sauvegarde le compte actif en tâche de fond (pour pouvoir y
  // revenir) et on ouvre TOUT DE SUITE la connexion/inscription. Se connecter à un autre compte
  // ne déconnecte pas le 1er : les jetons de chacun sont conservés (switch sans déconnexion).
  const addAccount = () => {
    void upsertTokens(session);
    setAddingAccount(true); // autorise l'accès à /login même connecté
    onClose();
        ...
        <Pressable style={styles.sheet} onPress={(e) => e.stopPropagation()}>
          <SafeAreaView edges={['bottom']}>
                ...
                return (
                  <Pressable key={a.id} onPress={() => onSwitch(a)} style={styles.row}>
                    ...
                    {switching === a.id ? (
                      ...
                      <Ionicons name="checkmark-circle" size={24} color={Afylo.violet} />
                    ) : (
                      <Pressable onPress={() => onRemove(a.id)} hitSlop={10}>
                        <Ionicons name="close" size={20} color={Afylo.textFaint} />
                      </Pressable>
                    )}
                  </Pressable>
                );
            ...
            <Pressable onPress={addAccount} style={styles.addRow}>
              <View style={styles.addIcon}><Ionicons name="add" size={22} color={Afylo.violet} /></View>
...
const styles = StyleSheet.create({
  ...
  title: { color: Afylo.text, fontFamily: Font.bold, fontSize: 18, marginBottom: 10 },
  row: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 10 },
  name: { color: Afylo.text, fontFamily: Font.semibold, fontSize: 15 },
  handle: { color: Afylo.textDim, fontSize: 13, marginTop: 1 },
  addRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 14, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: Afylo.border, marginTop: 4 },
  addIcon: { width: 44, height: 44, borderRadius: 22, borderWidth: 1.5, borderColor: Afylo.violet, borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center' },
  addText: { color: Afylo.violet, fontFamily: Font.semibold, fontSize: 15 },
});
