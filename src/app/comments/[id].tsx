...
type Comment = {
  ...
  gift?: number; // cadeau offert (FCFA) — l'argent va au créateur (5% Afylo)
...
export default function Comments() {
  ...
  // Cadeau : l'argent va au créateur (5% Afylo). Apparaît dans le fil.
          ...
          <SafeAreaView edges={['bottom']} style={styles.inputSheet}>
            ...
            <View style={styles.inputBar}>
              ...
              <Pressable onPress={() => { if (gate('offrir un cadeau')) setGiftOpen(true); }} style={styles.giftBtn}>
                <Ionicons name="gift" size={22} color={Afylo.gold} />
              </Pressable>
              {text.trim() ? (
                ...
                <Pressable onLongPress={startRec} delayLongPress={200} onPressOut={stopRecAndSend} style={[styles.send, recording && { backgroundColor: Afylo.live }]}>
    ...
    <View style={{ marginLeft: depth * 44, marginBottom: 16 }}>
      <View style={{ flexDirection: 'row' }}>
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          {c.voice ? (
            ...
            <View style={styles.giftNote}>
              <Ionicons name="gift" size={15} color={Afylo.gold} />
          ...
          <View style={styles.cActions}>
            ...
            {canDelete && (
              <Pressable onPress={() => onDelete(c.id)}>
                <Text style={[styles.cReply, { color: Afylo.live }]}>Supprimer</Text>
              </Pressable>
            )}
        ...
        <Pressable onPress={() => onLike(c.id)} style={styles.cLike}>
          <Ionicons name={c.liked ? 'star' : 'star-outline'} size={17} color={c.liked ? Afylo.gold : Afylo.textDim} />
...
const styles = StyleSheet.create({
  ...
  giftNote: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: Afylo.gold + '1A', borderWidth: 1, borderColor: Afylo.gold + '55', borderRadius: Radius.md, paddingHorizontal: 10, paddingVertical: 7, marginTop: 4, alignSelf: 'flex-start' },
  giftNoteText: { color: Afylo.gold, fontFamily: Font.bold, fontSize: 13 },
  overlay: { flex: 1, backgroundColor: 'transparent', justifyContent: 'flex-end' },
  topMedia: { flex: 1 },
  sheet: { height: '60%', backgroundColor: Afylo.bg, borderTopLeftRadius: 24, borderTopRightRadius: 24, overflow: 'hidden' },
