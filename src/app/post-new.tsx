        ...
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.kindRow}>
            {KINDS.map((k) => (
              <Pressable key={k.key} onPress={() => setKind(k.key)} style={[styles.kind, kind === k.key && styles.kindOn]}>
                <Ionicons name={k.icon as any} size={18} color={kind === k.key ? '#fff' : Afryko.textDim} />
          ...
          <View style={styles.mediaBox}>
            {mediaUrl ? (
              ...
              <>
                <Ionicons name="cloud-upload-outline" size={30} color={Afryko.textFaint} />
            ...
            {products.map((p) => {
              ...
              return (
                <Pressable key={p.id} onPress={() => toggle(p.id)} style={[styles.prodChip, on && styles.prodChipOn]}>
                  ...
                  <Ionicons name={on ? 'checkmark-circle' : 'ellipse-outline'} size={22} color={on ? Afryko.violet : Afryko.textFaint} />
                </Pressable>
              );
            })}
          </View>
...
const styles = StyleSheet.create({
  ...
  soundBtn: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: Afryko.surface, borderWidth: 1, borderColor: Afryko.border, borderRadius: Radius.md, paddingHorizontal: 14, height: 50, marginBottom: 12 },
  ...
  kind: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 11, borderRadius: Radius.pill, backgroundColor: Afryko.surface, borderWidth: 1, borderColor: Afryko.border },
  kindOn: { backgroundColor: Afryko.violet, borderColor: Afryko.violet },
  kindText: { color: Afryko.textDim, fontWeight: '700', fontSize: 13 },

  mediaBox: { height: 150, borderRadius: Radius.lg, borderWidth: 1.5, borderStyle: 'dashed', borderColor: Afryko.border, backgroundColor: Afryko.surface, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  mediaHint: { color: Afryko.textDim, fontSize: 13, marginTop: 8 },
  mediaHintSmall: { color: Afryko.textFaint, fontSize: 11, marginTop: 2 },

  input: { backgroundColor: Afryko.surface, borderRadius: Radius.md, borderWidth: 1, borderColor: Afryko.border, color: Afryko.text, fontSize: 15, paddingHorizontal: 14, height: 50, marginTop: 12 },
