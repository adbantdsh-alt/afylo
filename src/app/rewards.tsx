...
const OTHER = [
  ...
  { icon: 'briefcase', title: 'Partenariats', sub: 'Marques via Afylo' },
] as const;
  ...
  return (
    <View style={styles.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, paddingBottom: 48 }}>
        {/* Hero + gains du mois */}
        <LinearGradient colors={[Afylo.violet, Afylo.violet2]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.hero}>
          <View style={styles.heroBadge}><Ionicons name="cash" size={13} color="#fff" /><Text style={styles.heroBadgeText}>Afylo Creator Rewards</Text></View>
        ...
        <View style={styles.card}>
          ...
          {checks.map((c) => (
            <View key={c.key} style={styles.checkRow}>
              <Ionicons name={c.ok ? 'checkmark-circle' : 'ellipse-outline'} size={20} color={c.ok ? '#16A34A' : Afylo.textFaint} />
              <Text style={styles.checkLabel}>{c.label}</Text>
              <Text style={[styles.checkValue, { color: c.ok ? Afylo.text : Afylo.textDim }]}>{c.value}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.card}>
          {OTHER.map((o, i) => (
            <View key={o.title} style={[styles.otherRow, i < OTHER.length - 1 && styles.otherBorder]}>
              <View style={styles.otherIcon}><Ionicons name={o.icon as any} size={18} color={Afylo.violet} /></View>
              <View style={{ flex: 1 }}>
        ...
        <View style={styles.noteCard}>
          <Ionicons name="shield-checkmark" size={18} color={Afylo.violet} />
          <Text style={styles.noteText}>Afylo détecte bots, achats de vues, faux abonnés et fermes à clics. Les revenus frauduleux sont annulés et le compte peut être suspendu du programme.</Text>
        </View>
      </ScrollView>
    </View>
  );
...
function Section({ title, icon, children }: { title: string; icon: keyof typeof Ionicons.glyphMap; children: React.ReactNode }) {
  return (
    <View style={styles.card}>
      <View style={styles.rowBetween}><Text style={styles.cardTitle}>{title}</Text><Ionicons name={icon} size={18} color={Afylo.textDim} /></View>
      {children}
    </View>
  );
}
function Bullet({ text, good }: { text: string; good?: boolean }) {
  return (
    <View style={styles.bullet}>
      <Ionicons name={good ? 'checkmark' : 'close'} size={15} color={good ? '#16A34A' : Afylo.live} style={{ marginTop: 2 }} />
...
const styles = StyleSheet.create({
  ...
  title: { flex: 1, color: Afylo.text, fontSize: 20, fontFamily: Font.bold },
  ...
  heroBtnText: { color: Afylo.violet, fontFamily: Font.bold, fontSize: 15 },

  card: { backgroundColor: Afylo.surface, borderWidth: 1, borderColor: Afylo.border, borderRadius: Radius.lg, padding: 16, marginTop: 14 },
  sectionTitle: { color: Afylo.text, fontSize: 16, fontFamily: Font.bold, marginTop: 22 },
  cardTitle: { color: Afylo.text, fontSize: 16, fontFamily: Font.bold },
  ...
  checkLabel: { flex: 1, color: Afylo.text, fontSize: 14 },
  checkValue: { fontFamily: Font.semibold, fontSize: 13 },

  big: { color: Afylo.violet, fontSize: 30, fontFamily: Font.bold, marginTop: 4 },
  bigSub: { color: Afylo.textDim, fontSize: 14, fontFamily: Font.medium },
  exRow: { flexDirection: 'row', marginTop: 16, gap: 10 },
  ex: { flex: 1, backgroundColor: Afylo.surfaceAlt, borderRadius: Radius.md, paddingVertical: 12, alignItems: 'center' },
  exV: { color: '#16A34A', fontFamily: Font.bold, fontSize: 15 },
  exL: { color: Afylo.textDim, fontSize: 11, marginTop: 2 },

  bullet: { flexDirection: 'row', gap: 8, paddingVertical: 5 },
  bulletText: { flex: 1, color: Afylo.textDim, fontSize: 13, lineHeight: 19 },

  p: { color: Afylo.textDim, fontSize: 14, lineHeight: 20 },
  payRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 },
  payTag: { backgroundColor: Afylo.surfaceAlt, paddingHorizontal: 12, paddingVertical: 7, borderRadius: Radius.pill },
  payTagText: { color: Afylo.text, fontFamily: Font.semibold, fontSize: 12 },

  otherRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 11 },
  otherBorder: { borderBottomWidth: 1, borderBottomColor: Afylo.surfaceAlt },
  otherIcon: { width: 40, height: 40, borderRadius: 12, backgroundColor: '#3E5BFF14', alignItems: 'center', justifyContent: 'center' },
  otherTitle: { color: Afylo.text, fontFamily: Font.semibold, fontSize: 15 },
  otherSub: { color: Afylo.textDim, fontSize: 13, marginTop: 1 },
  ...
  noteText: { flex: 1, color: Afylo.textDim, fontSize: 13, lineHeight: 19 },
});
